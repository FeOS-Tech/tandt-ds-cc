...
// Register models with ti_easy_service database
    ...
    this.models.IssueMaster = connection.model('IssueMaster', issueMasterSchema);
  ...
  async createIssueMaster(issueData) {
    try {
      ...
      logger.error('Error creating IssueMaster:', error);
      throw error;
    }
  }

  async updateIssueMaster(ticketNumber, updateData) {
    try {
      const issue = await this.models.IssueMaster.findOneAndUpdate(
        { ticketNumber },
        updateData,
        { new: true }
      );
      logger.info('IssueMaster updated:', { ticketNumber });
      return issue;
    } catch (error) {
      logger.error('Error updating IssueMaster:', error);
      throw error;
    }
  }
  ...
  async completeServiceBooking(userData, session) {
    try {
      ...
      // 2. Create Issue Master
      const issueMaster = await this.createIssueMaster({
        brandName: this.getBrandName(session?.selectedBrand),
        issueType: this.getIssueType(session?.selectedIssue),
      ...
      logger.info('Service booking completed in MongoDB', {
        ...
        ticketNumber: issueMaster.ticketNumber
      });

      return {
        userMasterUpdated: true,
        issueMasterCreated: true,
        ticketNumber: issueMaster.ticketNumber
      };
    } catch (error) {
  ...
  getIssueType(issueId) {
    ...
    return issueMap[issueId] || issueId;
  }
  ...
  async getUserTickets(phoneNumber) {
    try {
      return await this.models.IssueMaster.find(
